...
class DiffAttention(nn.Module):
    ...
    """
    def __init__(self, q_dim, kv_dim, out_dim, num_heads=8, qkv_bias=True, attn_drop=0., proj_drop=0., lambda_init=0.8):
        super().__init__()
        if num_heads % 2 != 0:
        ...
        self.q_head_dim = q_dim // num_heads       # per-head dimension
        ...
        self.k_proj = nn.Linear(kv_dim, kv_dim, bias=qkv_bias)
        self.v_proj = nn.Linear(kv_dim, kv_dim, bias=qkv_bias)
        self.out_proj = nn.Linear(kv_dim, out_dim, bias=True)  # final output projection

        self.attn_drop = nn.Dropout(attn_drop)
        self.proj_drop = nn.Dropout(proj_drop)

        # RMSNorm for headwise normalization on outputs (each head's output has dimension 2·head_dim)
        self.diff_norm = RMSNorm(2 * self.kv_head_dim, eps=1e-5, elementwise_affine=True)

        # Learnable lambda parameters (shared across all heads)
        self.lambda_q1 = nn.Parameter(torch.zeros(self.q_head_dim, dtype=torch.float32).normal_(mean=0, std=0.1),requires_grad=True)
        self.lambda_k1 = nn.Parameter(torch.zeros(self.kv_head_dim, dtype=torch.float32).normal_(mean=0, std=0.1), requires_grad=True)
        self.lambda_q2 = nn.Parameter(torch.zeros(self.q_head_dim, dtype=torch.float32).normal_(mean=0, std=0.1),requires_grad=True)
        self.lambda_k2 = nn.Parameter(torch.zeros(self.kv_head_dim, dtype=torch.float32).normal_(mean=0, std=0.1),requires_grad=True)
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        k = self.k_proj(y)  # shape: (B, N, d_model)
        v = self.v_proj(y)  # shape: (B, N, d_model)

        # Reshape Q and K into (B, N, 2 * h_effective, head_dim)
        q = q.view(B, Q_N, 2 * self.effective_heads, self.q_head_dim)
        k = k.view(B, KV_N, 2 * self.effective_heads, self.kv_head_dim)
        # Reshape V into (B, N, h_effective, 2 * head_dim)
        v = v.view(B, KV_N, self.effective_heads, 2 * self.kv_head_dim)
        ...
        attn_probs = attn_probs.view(B, self.effective_heads, 2, Q_N, KV_N)
        ...
        attn_output = attn_output.transpose(1, 2).reshape(B, KV_N, 2 * self.effective_heads * self.kv_head_dim)
